// Living OS (Wave G2) — the Time Machine / Memory Replay core. PURE + DETERMINISTIC:
// every output is computed from data the system already has (Event Store, captures,
// nodes, edges, timeline, briefs, decisions). NO model calls — so it works identically
// in stub/live/replay mode. An optional AI narration layer can ride on top later, but
// the substance here never depends on live LLM completion.

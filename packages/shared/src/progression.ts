// Living OS (Wave G4) — Progression engine. PURE + DETERMINISTIC + EXPLAINABLE: XP,
// skill tracks, levels and project momentum are all computed by transparent rules from
// data the system already has (quests, nodes, edges, projects). No LLM. Every number
// can be traced to its inputs; the API also appends an xp_ledger row + event per grant.

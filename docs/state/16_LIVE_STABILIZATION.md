...
| 6 | **App-wide item management** | ✅ **AI results** (Activity: View/Open-source/Retry/Archive/Delete) + **nodes** (Create quest, Copy) — with confirm on destructive actions. Node **DELETE** backend already exists (`DELETE /nodes/:id`). **Recommended-next:** a single reusable `ItemActions` menu across captures/quests/briefs/timeline/context/projects with Edit/Rename/Move/Link/Mark-secret/Duplicate + soft-delete + undo toast (large, careful surface — scoped as its own pass). |
